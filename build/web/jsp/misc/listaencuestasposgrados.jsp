<%-- 
    Document   : listaencuestasposgrados
    Created on : 30/06/2017, 03:35:15 PM
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <p><font face="Arial, Helvetica, sans-serif" size="2"><br>
                            Para la Escuela Colombiana de Ingeniería Julio Garavito es muy importante conocer su opinión con respecto a los procesos académicos y sobre 
                            los actores (profesores y cursos) involucrados en ellos.  Por tal razón, le solicitamos diligenciar la encuesta académica para los cursos 
                            en los cuales está inscrito en el periodo actual.</font></p>
                        <p><font face="Arial, Helvetica, sans-serif" size="2">Tenga en cuenta que esta encuesta es anónima, por lo tanto sus datos personales no quedan relacionados
                            con las respuestas.</font></p>
                    </td>
                </tr>
                <tr>
                        ...
                        <fieldset>
                            ...
                            <center>
                                <legend><font color="#666666" size="2"></font></legend> 
                                    <% Vector encuestas = emisc.getlisEncuestaPosgrado();
                                        String iden = "";
                                        String descripcion = "";
                                        String prog = "";
                            ...
                            <form name="form1" method="post" action="FormatoEncuestaPosgrados">
                                <center>
                                    <select name="datos">
                                        <% for (int j = 0; j < encuestas.size(); j++) {
                                                Vector enc = (Vector) encuestas.elementAt(j);
                                                iden = enc.elementAt(3).toString();
                                                prog = enc.elementAt(4).toString();
                                                id_encuestado = enc.elementAt(2).toString();
                                                per = enc.elementAt(5).toString();
                                                tipo = enc.elementAt(6).toString();
                                                asignatura = enc.elementAt(1).toString();
                                                nom_prof = enc.elementAt(7).toString();
                                                ngrp = enc.elementAt(8).toString();
                                                
                                                
                                        %>
                                        <option value="<%=enc.elementAt(0)%>*<%=iden%>*<%=prog%>*<%=id_encuestado%>*<%=per%>*<%=tipo%>*<%=asignatura%>*<%=ngrp%>"> <%=enc.elementAt(1)%>-<%=nom_prof%></option>
                                        <%}%>
                                    </select>
                                </center>
                                <br>
