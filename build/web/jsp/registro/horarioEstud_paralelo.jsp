...
<%
    String id_nuevo = request.getParameter("id_nuevo");
    if(id_nuevo==null || (id_nuevo.trim()).equals("")){
%>
...
<%
            ...
            if(!(id_nuevo==null || (id_nuevo.trim()).equals(""))){
                est=new Estudiante(id_nuevo, oout);
            }
            configECI confEci = new configECI();
            String firmaacta= new String();
            Vector vh;%>

<%
            if(id_nuevo==null || (id_nuevo.trim()).equals("")){
                ActaJava acta = new ActaJava();
                int actajava = acta.getActaIntermedio(est.getCarnet(), confEci.getPeriodoHor());
                if (actajava < 0 ) {
                    response.sendRedirect("menuServEstud");
                } else if (actajava == 0) {
                    if (confEci.getPeriodoHor().contains("I")) {
                        response.sendRedirect("matriculaEstIntermedio?per=" + confEci.getPeriodoHor());
                    } else {
                        response.sendRedirect("matriculaEstudJava");
                    }
                }
            }
            Vector horario = est.getInfoHorarioPer("2020-1");%>
                                ...
                                <td align="center" valign="top" colspan="2">
                                    ...
                                    <%Vector horas;
                                        ...
                                        for (int j = 0; j < horario.size(); j++) {
                                            ...
                                            firmaacta = horarioest.elementAt(8).toString();
                                            
                                            marqueHora(horas, horarioest);
                                        }
                                    %>
                                    <%if (!firmaacta.equals("No disponible")){%>
                                    ...
                                    <%} else{%>
                                     <p>&nbsp;</p><p align="center"><b><font size="2" face="Verdana, Arial, Helvetica, sans-serif">Debe firmar primero el acta de matrícula</font></b></p>
