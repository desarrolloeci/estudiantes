<%-- 
    Document   : TarjetonPosgrados
    Created on : 19/10/2017, 09:00:28 AM
...
<html>
    <head>
        <title>Votación Comité Central Posgrados</title>
    ...
    <body bgcolor="#FFFFFF">
        ...
        <form method="post" action="GuardaVotosPosgrado" name="formulario1">
            <!-- Tabla externa -->
            ...
            <%  if (votacion.size() <= 0) {
                    out.println("No esta autorizado para realizar este proceso");
                } else {
                    for (j = 0; j < votacion.size(); j++) {

                        Vector dato = (Vector) votacion.elementAt(j);

                        //Vector dato = (Vector) votacion.elementAt(0);
                        String valida = dato.elementAt(8).toString();
                        if (valida.equals("1")) {
                            out.println("<center>");
                            out.println("<p class=\"textocom\">Su voto ya se encuentra registrado. Gracias por su participación.</p>");
                            out.println("<p>");
                        } else {
